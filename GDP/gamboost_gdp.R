...
var_predictors = paste("bspatial(", var_predictors, ", center=TRUE)", sep = '')
var_formula = as.formula(paste("y ~", paste(var_predictors, collapse = "+"), "+ bols(const, intercept=FALSE)"))   # build formula

var_gamboost_model = gamboost(formula = var_formula, data = var_model_data_boost, control = boost_control(mstop = 15))
...
var_gamboost_model = gamboost(y_t ~ bspatial(y_t_hat), data = var_boosting, control = boost_control(mstop = 1000))
gamboost_predict = predict(var_gamboost_model)
# Sum of VAR(p) forecast and boosting components
gamboost_forecast_y_h = prdVAR$fcst$logGDP[h] + (gamboost_predict[length(gamboost_predict)] - 
                                                   var_boosting[ (dim(var_boosting)[1]) , 2] )

# gamboost_forecast_y = cbind(gamboost_forecast_y, gamboost_forecast_y_h)
gamboost_forecast_y = gamboost_forecast_y_h
##############################

rm(list = c("gamboost_forecast_temp_h", "gamboost_forecast_y_h", "var_formula", "var_predictors", "gamboost_predict",
            "var_model_data_boost", "var_boosting", "prdVAR", "var", "VARsel", "var_gamboost_model" ))
