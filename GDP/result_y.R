...
forecast_df = cbind(df_var_boost_forecast, 
                  (df_var_boost_forecast[,1]-df_var_boost_forecast[,2])^2, 
                  (df_var_boost_forecast[,1]-df_var_boost_forecast[,3])^2, 
                  (df_var_boost_forecast[,1]-df_var_boost_forecast[,4])^2)

MSE_var = mean(forecast_df[,5])
MSE_gamboost_y = mean(forecast_df[,6])
MSE_gamboost_full = mean(forecast_df[,7])

forecast_ts = ts(forecast_df, start=c(2000, 1), frequency = 4)
colnames(forecast_ts) = c("true GDP", "VAR", "VAR + GAMboost(Y)", "VAR + GAMboost(Full)",
                        "SqE VAR", "SqE GAMBoost(Y)", "SqE GAMBoost(Full)")
write.csv(forecast_ts, "/Users/antongorev/Dropbox/Study/Seminar/my tests/GDP/log_diff_d1.csv")
print(forecast_ts)

MSEs = c(MSE_var, MSE_gamboost_y, MSE_gamboost_full )
names(MSEs) = c("MSE_VAR", "MSE_GAMboost(Y)", "MSE_GAMBoost(Full)")
